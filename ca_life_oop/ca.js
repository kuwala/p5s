function CA_Life() {
  ...
  this.generate = function () {
    ...
    for (var i = 0; i < this.activeCells.length; i++) {
      ...
      for (var j = 0; j < column.length; j++) {
        ...
        this.cells[i][j].newState( this.rules(i,j) );
  ...
  this.countNeighbors = function (x,y) {
    ...
    // Top
    if(this.activeCells[left][top].previousState == 1) { neighbors += 1}
    if(this.activeCells[x][top].previousState == 1) { neighbors += 1}
    if(this.activeCells[right][top].previousState == 1) { neighbors += 1}

    // Sides
    if(this.activeCells[left][y].previousState == 1) { neighbors += 1}
    if(this.activeCells[right][y].previousState == 1) { neighbors += 1}

    // Bottom
    if(this.activeCells[left][bot].previousState == 1) { neighbors += 1}
    if(this.activeCells[x][bot].previousState == 1) { neighbors += 1}
    if(this.activeCells[right][bot].previousState == 1) { neighbors += 1}
  ...
  this.rules = function (x,y) {
    ...
    var cellState = this.activeCells[x][y].previousState; // 0 dead, 1 alive
